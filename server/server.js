...
import serverRoutes from './routers/index';
...
if(session?.scope && session?.shop)
{
  console.log('ACTIVEEEEE SESSSSSSSSSSSSION ', session);
...
app.prepare().then(async () => {
  ...
  //my middlewears START
  async function injectSession(ctx, next) {
    const session = await Shopify.Utils.loadCurrentSession(ctx.req, ctx.res);
    ctx.sessionFromToken = session;
    // if(!session)
      // console.log("Middlewear", session);
    // else
    // console.log("Middlewear ctx after injecting session ", ctx);
    return next();
  }
